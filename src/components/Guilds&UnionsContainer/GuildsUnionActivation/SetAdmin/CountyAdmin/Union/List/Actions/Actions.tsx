...
import { useRemoveAdminInUnion } from "../../../../../../../../core/services/api";
...
const Actions: React.FC<IPropTypes> = ({
  ...
  const removeMutation = useRemoveAdminInUnion()
      ...
      .then((result) => {
        if (result.isConfirmed) {
                  removeMutation.mutate(
                    { unionId: parsed.id, userId: id },
                    {
                      onSuccess: (val: any) => {
                        showToast(["با موفقیت انجام شد."], ToastTypes.success);
                        const newEvent = { ...refetchEvent };
                        newEvent.unionAdminRemove = !newEvent.unionAdminRemove;
