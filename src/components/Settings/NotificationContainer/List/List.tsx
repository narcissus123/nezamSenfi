...
const List = () => {
  ...
  const getListMutation = useGetMessageSettingHistory();
  ...
  }, [pageSize, refetchEvent.notificationHistoryList]);
    ...
    if (getListMutation.data && getListMutation.data.data.result) {
      if (getListMutation.data && getListMutation.data.data.result) {
        ...
        items.forEach((row: any) => {
          newData.push({
            id: row.id,
            isSmsActive: row.isSmsActive ? "بله" : "خیر",
            isEmailActive: row.isEmailActive ? "بله" : "خیر",
            isTicketActive: row.isTicketActive ? "بله" : "خیر",
