...
import { useGetAllJobCategoryTariffs, useGetInspectionInspectionTypeLandAreaModel } from "../../../../../core/services/api";
...
const List: FC<IPropTypes> = ({ activeTariff, activeTariffSuccess }) => {
  ...
  const getMutation = useGetAllJobCategoryTariffs();
  ...
  const { data, isLoading, isSuccess } =
    useGetInspectionInspectionTypeLandAreaModel();
  ...
  useEffect(() => {
    if (data && data.data) {
      ...
      data.data.result.forEach((row: any) => {
        options.push({
          value: row.inspectionTableTypeLandArea,
          label: row.title,
        });
      });
  ...
  }, [pageSize, activeTariffSuccess, refetchEvent.jobCategoryTariffList]);
