...
import {
  ...
  useSelectUnionRequestByIdBySecretariat,
  useGetAllUnionRequestHistory,
  useGetUnionRequestDetailsForItManager,
...
const Union: FC<IPropTypes> = ({
  ...
  const getDetails = useGetUnionRequestDetails();
  const getItManagerDetails = useGetUnionRequestDetailsForItManager();
  const { id }: any = useParams();
  const [initialValue, setInitialValue] = useState<any>(null);
  const selectMutation = useSelectUnionRequestByIdBySecretariat();
  ...
  useEffect(() => {
    if (isItManagerCartable) getItManagerDetails.mutate(id);
  ...
  return (
    <CheckGuildsActivation
      ...
      historyMutation={useGetAllUnionRequestHistory}
