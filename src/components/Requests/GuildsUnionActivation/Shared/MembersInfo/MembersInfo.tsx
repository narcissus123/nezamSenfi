...
const MembersInfo: FC<IPropTypes> = ({
  ...
  useEffect(() => {
    if (requestDetail && requestDetail.users) {
      setTableData([]);
      requestDetail.users.forEach((row: any) => {
        let newFormObject = {
          id: row.userId,
  ...
  const onFinalSubmit = (value: any) => {
    if (!value.files || !(value.files.length > 0)) {
      ...
      setMembersMutation.mutate(formData, {
        onSuccess: (val: any) => {
          const curStatus = requestDetail.status;
          showToast(["با موفقیت انجام شد"], ToastTypes.success);
          refetch.mutate(+id);
          if (curStatus !== 7) {
            GoToTruePage(2, `${type}ApplicantFlow`, id);
          }
        },
      });
    }
          ...
          >
            {({ values, handleChange, setFieldValue }) => (
              <Form>
                ...
                <StatusWrapper
                  curStatus={[requestDetail ? requestDetail.status : 1]}
                  guildStatus={[
                    ...
                    GuildsActivation.AddedBankInfo,
                  ]}
                >
                  <SubmitButton
