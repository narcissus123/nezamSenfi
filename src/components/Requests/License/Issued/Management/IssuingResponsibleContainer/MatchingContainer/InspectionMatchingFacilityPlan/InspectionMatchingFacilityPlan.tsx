...
const InspectionMatchingFacilityPlanContainer: FC = () => {
  ...
  const { data, isFetching, isSuccess } =
    useGetBuildingOfLicenseReuestSectionByIssuingResponsible(+section_id);

  const getLisenceDetails = useGetBuildingSectionMap();
  const setMutation = useRegisterBuildingSectionMap();
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>ثبت موقعیت تاسیسات قطعه درخواست پروانه</CardTitle>
      </CardHeader>
      <CardBody>
        <FormGroup
        ...
        >
          <LoadScriptNext
            ...
            version="weekly"
          >
            {getSection.isLoading || isFetching ? (
              <FallBackSpinner />
            ) : (
              <GoogleMap
                mapTypeId="hybrid"
        ...
        <Row style={{ marginTop: "25px" }}>
          <Col>
            <SimpleSubmitButton
              isLoading={false}
              btnText="ثبت موقعیت تاسیسات قطعه"
              onCLick={onSubmit}
            />
          </Col>
        </Row>
...
export { InspectionMatchingFacilityPlanContainer };
