...
import { columns } from "./MainLocationManagerJobRequestListColumn";

interface IPropTypes {
  getManagerCartable?: any;
}

const MainLocationManagerJobRequestList: FC<IPropTypes> = ({
  getManagerCartable,
}) => {
  ...
  return (
    <SecretariatList
      useMutate={
        getManagerCartable ? getManagerCartable : getAllMainLocationPosition
      }
      columns={columns}
      getOwnedMainLocationGuildRoom={getOwnedMainLocationGuildRoom}
      getCustomProps={{ flow: "MainLocationManagerJobRequestFlow" }}
    />
  );
};

export { MainLocationManagerJobRequestList };
