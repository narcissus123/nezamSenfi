import React, { useState } from "react";
import { ListGroup, ListGroupItem, Spinner } from "reactstrap";
import Styled from "./ResumeDetails.module.scss";
import {
  useServeFileByAdmins,
  useServeShowFileByAdmin,
} from "../../../../../../../../../core/services/api";
import { DownloadRow } from "../../../../../../../../common/DownloadRow/DownloadRow";

interface IPropTypes {
  data: any;
}

const ResumeDetails: React.FC<IPropTypes> = ({ data }) => {
  const [isLoading, setIsLoading] = useState<any>(false);
  console.log("daaataa----", data);

  return (
    <>
      <ListGroupItem tag="a" active>
        جزئیات رزومه
      </ListGroupItem>

      <ListGroup className="list-group-horizontal-sm">
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          نام سازمان: {data.organizationTitle}
        </ListGroupItem>
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          وضعیت بیمه: {data.inSuranceStatus}
        </ListGroupItem>
      </ListGroup>
      <ListGroup className="list-group-horizontal-sm">
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          استان محل خدمت: {data.provinceTitle}
        </ListGroupItem>
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          مدت سابقه بیمه ای: {data.inSuranceDuration}
        </ListGroupItem>
      </ListGroup>
      <ListGroup className="list-group-horizontal-sm">
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          شهرستان محل خدمت: {data.countyTitle}
        </ListGroupItem>
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          نوع اشتغال: {data.positionTypeTilte}
        </ListGroupItem>
      </ListGroup>
      <ListGroup className="list-group-horizontal-sm">
        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          تاریخ شروع فعالیت: {data.startDate}
        </ListGroupItem>

        <ListGroupItem tag="a" className={Styled["item-flex"]}>
          نوع ارائه خدمت: {data.positionTitle}
        </ListGroupItem>
      </ListGroup>

      <ListGroup className="list-group-horizontal-sm">
        <ListGroupItem className={Styled["item-flex"]}>
          <div className="d-flex">
            <p>اسناد ارسالی &nbsp;</p>
            {isLoading && (
              <Spinner
                className={Styled["padding-top-spinner"]}
                color="success"
                size="sm"
              />
            )}
          </div>

          {data.resumeFilePaths ? (
            <>
              {data.resumeFilePaths.map((row: any, key: any) => {
                return (
                  <>
                    <DownloadRow
                      mutate={useServeFileByAdmins}
                      type="admin"
                      row={row}
                      // setIsShow={(val: boolean) => setIsOpen(val)}
                      useServeShowFile={useServeShowFileByAdmin}
                    />
                    <hr />
                  </>
                );
              })}
            </>
          ) : (
            <></>
          )}
        </ListGroupItem>
      </ListGroup>
    </>
  );
};

export { ResumeDetails };
