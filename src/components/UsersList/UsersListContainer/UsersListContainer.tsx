...
import { IRealUsersListFilter } from "../../../core/models";
import { columns } from "./realUserColumns";
import { useGetRealUsersList } from "../../../core/services/api";
import { RealUserListFilter } from "./RealUserListFilter";
...
const UsersListContainer: React.FC = () => {
  ...
  const RealUserListMutation = useGetRealUsersList();
  const [filterState, setFilterState] = useState<IRealUsersListFilter>({
    nationalCode: "",
    email: "",
    locationId: 0,
    name: "",
  ...
  useEffect(() => {
    RealUserListMutation.mutate({ ...filterState, pageSize: pageSize });
  }, [pageSize]);
  ...
  useEffect(() => {
    if (RealUserListMutation.isSuccess) {
      setState(RealUserListMutation.data?.data.result.users);
      setPageCount(
        Math.ceil(RealUserListMutation.data?.data.result.totalCount / pageSize)
      );
    }
  }, [RealUserListMutation.isSuccess]);
  ...
  const onSubmit = (value: any) => {
    const submitData: IRealUsersListFilter = {
      email: value.email,
    ...
    RealUserListMutation.mutate(submitData);
    setInitialPage(0);
  };
            ...
            <Card>
              <CardBody>
                <ListTable
                  ...
                  isLoading={RealUserListMutation.isLoading}
                  columns={columns}
